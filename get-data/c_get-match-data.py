...
# get matches
with open('c_matches.txt', 'r') as f:
...
def main():
    with open(r'c_match-data.csv', 'a') as f:
        ...
        while len(matches) > 0:
            ...
            try:
                ...
                response = requests.get(
                    'https://europe.api.riotgames.com/lol/match/v5/matches/' + matches[
                        0] + '/timeline?api_key=' + apiKey)
                data = response.json()

                lastEvent = data['info']['frames'][-1]['events'][
                    -1]  # so python doesnt have to iterate over the whole dictionary each time
                ...
                if lastEvent['timestamp'] > 870000:
                    ...
                    while x <= 15:  # get all infos until (including) 15th minute
                        ...
                        events = data['info']['frames'][x][
                            'events']  # so python doesnt have to iterate over the whole dictionary each time
                        ...
                        participants = data['info']['frames'][x][
                            'participantFrames']  # so python doesnt have to iterate over the whole dictionary each time
                ...
                writeToFile = [blueTeamWin, round(blueTeamWardsPlaced, 2), round(blueTeamWardsDestroyed, 2),
                               blueTeamTowerDestroyed,
                               blueTeamDragonsKilled,
                               blueTeamHeraldsKilled, blueTeamKills, blueTeamDeaths, blueTeamAssists,
                               blueTeamFirstBlood,
                               blueTeamTotalGold, round(blueTeamAvgLevel, 2), blueTeamTotalMinionsKilled,
                               blueTeamTotalJungleMonsterKilled, round(blueTeamCsPerMinute, 2),
                               round(blueTeamGoldPerMinute, 2),
                               redTeamWin,
                               round(redTeamWardsPlaced, 2), round(redTeamWardsDestroyed, 2), redTeamTowerDestroyed,
                               redTeamDragonsKilled,
                               redTeamHeraldsKilled, redTeamKills, redTeamDeaths, redTeamAssists, redTeamFirstBlood,
                               redTeamTotalGold,
                               round(redTeamAvgLevel, 2), redTeamTotalMinionsKilled, redTeamTotalJungleMonsterKilled,
                               round(redTeamCsPerMinute, 2),
                               round(redTeamGoldPerMinute, 2), gameDuration]
